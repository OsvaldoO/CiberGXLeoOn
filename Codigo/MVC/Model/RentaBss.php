...
class RentaBss{
	//Atributos
		 ...
		 */
		 ...
		 function realizar( $nick, $codigo, $fecha)
		 {
			...
			$query = "INSERT INTO 
						rentas (nick_cliente,cod_empleado,fecha) 
					VALUES 
						('$nick',
						'$codigo',
						'$fecha')"; 	
		...
		if($resultado == FALSE)
			...
			{
				$conexion->cerrarConexion();
				$renta = new RentaClass( $nick, $codigo, $fecha);
				return $renta; 
			}
		 }
		...
		function consultar( $clave )
		{
			...
			else
			{
				...
				{
					$renta = new RentaClass( $resultado[0]['nick_cliente'], $resultado[0]['cod_empleado'], $resultado[0]['fecha']);
					return $renta;
				}
				else 
					return FALSE; 
